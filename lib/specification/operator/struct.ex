defmodule Specification.Operator.Struct do
  defmacro __using__(opts) do
    ...
    quote do
      ...
      @behaviour Specification.Operator
      ...
      def evaluate(operator, value) do
        reduce_clauses(operator, with: &Specification.evaluate(&1, value))
      end

      def result(operator, value) do
        results = reduce_clauses(operator, with: &Specification.result(&1, value))
      ...
      defp reduce_clauses(operator, with: transformer) do
        ...
        transformed
        |> Enum.map(&Specification.passed?/1)
        |> passed?()
        |> if do
          {:ok, transformed}
        else
      ...
      defimpl Specification.Operator.Linkable, for: __MODULE__ do
