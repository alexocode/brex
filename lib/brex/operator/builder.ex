defmodule Brex.Operator.Builder do
  @moduledoc false
    ...
    quote do
      ...
      def evaluate(rule, value) do
        results =
          rule
          |> clauses()
          |> Enum.map(&Brex.evaluate(&1, value))
  ...
  def __after_compile__(%{module: module} = env, _bytecode) do
    defimpl_nestable(module) ||
      raise CompileError,
        ...
        line: env.line,
        description:
          "cannot use Brex.Operator " <>
            "on module #{inspect(module)} without defining aggregator/1 and clauses/1. " <>
            "Take a look at the `Brex.Operator` module documentation for more information."
  end
  ...
  defp defimpl_nestable(module) do
    ...
    if Enum.all?(expected_functions, &apply(Kernel, :function_exported?, [module | &1])) do
      defimpl Brex.Operator.Aggregatable, for: module do
